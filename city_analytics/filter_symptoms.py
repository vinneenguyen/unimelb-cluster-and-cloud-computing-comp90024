...
while True:	
	...
	print(timestamp)
	# lastdoc=symptoms_db.all_docs(descending=True,limit=1)
	# print(lastdoc)
	# if lastdoc['total_rows']!=0:
	# 	last_id=(lastdoc['rows'][0]['id'])
	# else:
	# 	last_id = 0

	#selector={'_id':{'$gte':last_id},'text':{'$regex':'(?i)(cough|fever|dry cough|smell loss|taste loss|sore throat|difficulty breathing|joint pain|bodyache|fatigue|vomit|diarrhoea|headache|tiredness)'}}
	...
	symptoms(db,symptoms_db,timestamp)
	view=View(ddoc_symptoms,'symptoms_location')
	# try:
	# 	view=View(ddoc_symptoms,'symptoms_location')
	# except:
	# 	symptoms_view(symptoms_db)
